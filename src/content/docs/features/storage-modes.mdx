...
- Files appear in both the Smarter Files component **and** the standard Files related list, Chatter, and any other native UI that reads `ContentDocumentLink`.
...
- Files are linked **only** through the custom junction object `Tucario_File_Visibility__c`.
- Files appear **only** in the Smarter Files component — they are hidden from the standard Files related list, Chatter, and any other native UI.
...
1. Starts a batch job (`TucarioMigrateFilesBatch`) that creates junction records for all existing `ContentDocumentLink` records on that record.
